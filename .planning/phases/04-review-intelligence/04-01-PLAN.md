...
**Note:** This plan touches 23 files but the work is largely mechanical (struct definitions, SQL DDL, CRUD boilerplate). Monitor context budget -- if approaching 50%, prioritize completing adapter implementations over exhaustive test coverage for edge cases.
</objective>
...
5. **000007_add_head_sha** -- Add head_sha to pull_requests:
   ...
   ```
   Down migration uses `ALTER TABLE pull_requests DROP COLUMN head_sha;`
   **NOTE:** `DROP COLUMN` requires SQLite 3.35.0+ (2021-03-12). The modernc.org/sqlite driver bundles SQLite 3.46.0+ so this is safe for our runtime. However, if the down migration is ever run against an external SQLite binary older than 3.35.0, it will fail. This is acceptable for a development tool -- down migrations are rarely executed in production, and our bundled driver exceeds the minimum version. Add a SQL comment in the down migration file:
   ```sql
...
`cd /home/esfisher/dev/mygitpanel && go test ./internal/adapter/driven/sqlite/... -v -count=1` -- all tests pass including new reviewrepo and botconfigrepo tests. `go build ./...` compiles. `go vet ./...` passes.
  ...
  <done>
5 migration pairs (10 SQL files) create reviews, review_comments, issue_comments, bot_config tables and add head_sha column. Migration 000007 down includes SQLite 3.35.0+ version comment for DROP COLUMN. ReviewRepo implements ReviewStore with full CRUD and passes 6+ test cases. BotConfigRepo implements BotConfigStore with seeded defaults and passes 5+ test cases. All existing tests still pass.
  </done>
</task>
